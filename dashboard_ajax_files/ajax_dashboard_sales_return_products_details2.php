...
if($_REQUEST['empid'] and $_REQUEST['empid2']) {
	$sql = "SELECT sum(HKSP.quantity) as quantity,HKP.name,HKP.type,HKP.quantity_type
          ...
          LEFT JOIN hk_sales_return_products AS HKSP ON HKSP.sales_return_id = HKSR.id
          LEFT JOIN `hk_products` AS HKP ON HKP.id = HKSP.product_id
          WHERE HKSR.date BETWEEN '".$_REQUEST['empid2']."' AND '".$_REQUEST['empid']."' GROUP BY HKSP.product_id";
