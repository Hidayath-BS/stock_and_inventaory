...
// person name should fetch form person table
...
$personQ = "SELECT first_name, last_name FROM hk_persons WHERE id=$person_id";

$personExe = mysqli_query($conn,$personQ);

while($personRow = mysqli_fetch_array($personExe)){
  $person_name = $personRow["first_name"]." ".$personRow["last_name"];
}


// sales return bill number
...
$purchaseReturnQuery = "INSERT INTO `hk_sales_return`
                        (`date`, `sales_id`, `amount_to_be_paid`, `transaction_type_id`, `cheque_number`, `transaction_id`, `sales_return_bill_number`, `amount_paid`, `due`, `transaction_table_id`,`person_id`)
                 VALUES ('$ondate',NULL,'$totalPay','$transType', NULL, NULL, '$salesbill', '$totalPaid', '$duepay', '$last_trans_id','$person_id')";
...
foreach($returnproduct as $returnRow){
    ...
    $stockTrackerQ = "INSERT INTO `hk_stock_tracker`(`product_id`, `date`, `particulars`, `add_stock`, `sub_stock`, `amount`) 
    VALUES ('$productid','$ondate','SALES RETURN','$quantity','0','$amount')";

    mysqli_query($conn,$stockTrackerQ);
...
function salesReturnAccEntry($ondate,$particulars,$invoice,$supplier_id,$credit_cash,$amount){
  ...
  $query = "INSERT INTO `hk_sales_return_account`(`date`, `particulars`, `bill_number`, `person_id`, `credit/cash`, `amount`)
