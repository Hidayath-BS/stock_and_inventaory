   ...
   $opening_balance = $previous_debit_total - $previous_credit_total;
    ...
    $query  = "SELECT * FROM `hk_account_".$customer_id."` WHERE date = '$actualOndate' AND active = '1'";
               ...
               for($x = 0; $x < count($print_array); $x++){
                 if(strpos($print_array[$x]["particulars"],"CR S on Bill No: ") !== false){
                   $sales_bill_no = substr($print_array[$x]["particulars"], 17);
                   $sales_bill_no = trim($sales_bill_no);
                   $query = "SELECT HKPR.name,HKPR.type,HKPR.quantity_type,HKSP.quantity,HKSP.rate,HKSP.amount
                              FROM `hk_sales` AS HKS
                              LEFT JOIN `hk_sales_products` AS HKSP ON HKSP.sales_id = HKS.id
                              LEFT JOIN `hk_products` AS HKPR ON HKSP.product_id = HKPR.id
                              WHERE HKS.bill_number = '$sales_bill_no'";
                 ...
                 else if(strpos($print_array[$x]["particulars"],"Credit Sales Return on Bill No: ") !== false){
                   $sales_return_bill_no = substr($print_array[$x]["particulars"], 32);
                   $sales_return_bill_no = trim($sales_return_bill_no);
                   $query = "SELECT HKP.name,HKP.type,HKP.quantity_type,HKSRP.quantity,HKSRP.rate,HKSRP.amount
                              ...
                              WHERE HKSR.sales_return_bill_number = '$sales_return_bill_no'";
                              $exe1 = mysqli_query($conn,$query);
                              $y = 0;
                              while($row1 = mysqli_fetch_array($exe1)){
                                $product_array[$x][$y]["product_name"] = $row1['name']." ".$row1['type'];
               ...
               $pdf->Cell(130,5,''.$supplier_name.' (Customer) Balance Sheet',0,1,'L');
               $pdf->Ln();
