...
if($_POST["city"] == "allcities"){
  ...
  $query  = "SELECT id,first_name,last_name FROM  `hk_persons` WHERE person_type_id = '2'";
...
for($x=0;$x<$person_array_length;$x++){
  ...
  $pdf->Cell(130,5,'Customer Balance Report',0,1,'L');
  $pdf->Ln();
  ...
  $pdf->Cell(110,8,'Customer Name','B',0,'L',true);
...
$x = 1;
            for($y=0;$y<count($print_array);$y++){
               ...
               $pdf->Cell(110,8,strtoupper($print_array[$y]["customer_name"]),0,0,'L',true);
               $pdf->Cell(50,8,money_format('%!i',$print_array[$y]["balance"]),0,1,'R',true);
               $x++;
             }

             $pdf->Cell(140,8,'Grand Total : ','T',0,'R',true);
  ...
  $query  = "SELECT id,first_name,last_name FROM  `hk_persons` WHERE person_type_id = '2' AND city_id = '$city_id'";
             $exe = mysqli_query($conn,$query);
             $person_array_length = mysqli_num_rows($exe);
               $x = 0;
             while($row = mysqli_fetch_array($exe)){
...
for($x=0;$x<$person_array_length;$x++){
 ...
 if($personBal < 0){
  ...
  $pdf->Cell(130,5,'Customer Balance Report of '.$city.' City',0,1,'L');
  $pdf->Ln();
  $pdf->Cell(20,5,'On Date : ',0,0,'L');
  $pdf->Cell(30,5,$date,0,1,'L');
